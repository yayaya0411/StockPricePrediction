...
def dnn(n_obs):
    ...
    # print('\n',n_obs[0],'\n')
    model = Sequential()
    model.add(Dense(units=1024, input_shape=[n_obs[1]], activation="linear"))
...
def conv1d(n_obs):
    ...
    padding = 'same'
    model = Sequential()
    model.add(Conv1D(filters = 128, kernel_size=kernel_size, strides=strides, padding=padding, activation = 'linear',input_shape=(n_obs[1],n_obs[2])))
...
def conv2d(n_obs):
    ...
    padding = 'same'
    model = Sequential()
    model.add(Conv2D(filters = 128, kernel_size=kernel_size,  padding=padding, activation = 'linear',input_shape=(n_obs[1],n_obs[2],1)))
...
def lstm(n_obs):
    model = Sequential()
    model.add(LSTM(128, return_sequences=True,activation = 'linear',input_shape=(n_obs[1],n_obs[2])))
